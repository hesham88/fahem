# Project Walkthrough - Version 43
...
2. **Directory Roles**:
   ...
   * `web/`: Next.js web application root (utilizing the Next.js **App Router**), hosted on **Firebase App Hosting** with continuous deployment (CD) via **GitHub**. Configured with Vanilla CSS styling system using a white creamish background and custom color palette (blue, orange, yellow, gold, white, black) in `src/app/globals.css`.
     ...
     * `/api/agent`: Streaming API route that resolves the Python executable path and spawns the Python agent to pipe execution output in real-time.
...
* **v41.0** (2026-05-29T07:25:00+03:00): Synchronized package lock dependencies for Firebase App Hosting deployment compilation, removing dependency mismatches for kerberos and mongodb-client-encryption. Verified with successful Next.js build compilation locally.
