# Project Walkthrough - Version 68
...
2. **Directory Roles**:
   ...
   * `web/`: Next.js web application root (utilizing the Next.js **App Router**), hosted on **Firebase App Hosting** with continuous deployment (CD) via **GitHub**. Configured with Vanilla CSS styling system using a white creamish background and custom color palette (blue, orange, yellow, gold, white, black) in `src/app/globals.css`.
     ...
     * **reCAPTCHA Enterprise Integration (Pending)**: Incorporate Google Cloud reCAPTCHA Enterprise with site key `6LfT9wQtAAAAAFElDHZ9ddSZHbKzMZx2-IO7PLKV` on form submissions and user logins. Implement server-side verification using the `google-cloud-recaptcha-enterprise` Python/Node SDK.
...
* **v66.0** (2026-05-31T04:40:46+03:00): Documented the premium circular concentric loading animation and floating background spheres implemented across all loading views.
* **v65.0** (2026-05-31T04:23:12.603852): Establish Phase 14 for Google Cloud reCAPTCHA Enterprise security integration.
* **v64.0** (2026-05-30T21:15:00+03:00): Registered mobile collapsible sidebar drawer layout details, active state variables, toggle close icons, and responsive media-query CSS overrides.
