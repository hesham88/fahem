# Project Walkthrough - Version 70
...
2. **Directory Roles**:
   ...
   * `scripts/`: Holds automation and standard scripts. Includes the automated `evaluate_compliance.py` audited script.
   ...
   * `web/`: Next.js web application root (utilizing the Next.js **App Router**), hosted on **Firebase App Hosting** with continuous deployment (CD) via **GitHub**. Configured with Vanilla CSS styling system using a white creamish background and custom color palette (blue, orange, yellow, gold, white, black) in `src/app/globals.css`.
     * **Firebase Phone Sign-In (New Feature)**: Inside `src/app/[locale]/page.tsx`, a sleek, cream-paper tab toggler allows users to transition between Google and Phone Authentication.
...
## Revision History
* **v70.0** (2026-05-31T06:30:00+03:00): Documented multilingual Firebase Phone Sign-In implementation, interactive reCAPTCHA verifiers, complete i18n synchronization, and clean production Next.js build compilation.
* **v69.0** (2026-05-31T06:15:00+03:00): Documented onboarding suggestions amnesia parsing fix, Places autocomplete school input field integrations, and premium categorized avatar selection chooser panel.
* **v68.0** (2026-05-31T05:53:00+03:00): Documented the dashboard side nav cleanup and integration of saved chats scroll panel & new chat button inside the floating companion chat UI (`StickyChat.tsx`).
* **v67.0** (2026-05-31T05:32:00+03:00): Documented the onboarding agent amnesia fix, proxy caching prevention, and multi-turn sequential validation.
