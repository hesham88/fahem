...
def main():
    ...
    if not be_data:
        ...
        if be_sha not in allowed_shas:
            # We can allow 'local' or 'unknown' for backend bypass if not baked in
            if be_sha in ["local", "unknown"]:
                print("[DEPLOY][WARN] Backend SHA is unknown/local; bypassing backend SHA check.")
            else:
