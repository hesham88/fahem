...
def get_gemini_embedding_v2(text, api_key, max_retries=6):
    """
    Fetches embeddings using the official google-genai SDK.
    Rides through transient rate-limits (429 / RESOURCE_EXHAUSTED) with exponential
    backoff so a momentary quota throttle never aborts the ingestion job.
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            last_err = e
            msg = str(e).lower()
            transient = ("429" in msg or "resource_exhausted" in msg or "rate" in msg
            ...
            if transient and attempt < max_retries - 1:
                ...
                wait = min(60, (2 ** (attempt + 1))) + random.uniform(0, 1.5)
                print(f"[Embedding Retry] transient error (attempt {attempt+1}/{max_retries}); backing off {wait:.1f}s: {e}",
                      file=sys.stderr, flush=True)
                time.sleep(wait)
                continue
            print(f"[Embedding Error] {e}", file=sys.stderr)
            break
    raise RuntimeError(f"Gemini embedding API call failed after {max_retries} attempts: {last_err}")
